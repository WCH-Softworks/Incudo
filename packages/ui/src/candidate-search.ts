 ...
 * one as a card does not fit and does not perform. This is the presentational half of that
 * problem — matching and a hard cap on how many are returned — kept here rather than in
 * `apps/desktop` because a card-stack mobile shell will face the identical list and the
 * identical cap (CODE-REUSE-POLICY rule 2). It knows nothing about elements or descriptions:
...
/**
 * `query` empty returns every option, alphabetically, capped at `limit` — the state a picker
 * opens in before the user types anything. A non-empty query ranks an exact label match first,
