...
The alternative — defining a clean Incudo format and asking the community to convert — has
been tried by every would-be Aurora successor and has never worked.

## Decision

- `@incudo/aurora-import` is built in **Phase 0**, before any UI. It is the first thing that
  runs, driven by the `incudo` CLI.
...
- Incudo's own model is designed to be a superset of Aurora's semantics, so import is
  ...
  guessing silently**.
- The importer never mutates upstream files. Fixes go in Incudo overlay files.

## Consequences

**Good**
- Day one, Incudo has more content than any greenfield competitor.
- AuroraLegacy keeps maintaining content; Incudo gets those updates for free.
...
**Bad / accepted**
- Incudo inherits Aurora's design decisions, including its quirks. Some of that is technical
