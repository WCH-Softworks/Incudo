...
`@heroforge/aurora-import`. There is no official schema — everything here is observed.
...
**This list is exactly what HeroForge turns into data.** In HeroForge these are declared by the
game system definition (`systems/dnd5e/system.json`), which is the whole trick that makes the
engine system-agnostic: Aurora hardcodes them, HeroForge reads them.
...
Precedence observed: `!` > `,` (and) > `||` (or). `@heroforge/core`'s
...
`companion:hp:max`, `wizard:spellcasting:prepare`, `strength:save:misc`. HeroForge keeps the
...
- Aurora's `<description>` is loose HTML with app-specific bits (`<div class="reference">`,
  `<div element="ID_…"/>`). HeroForge stores it as HTML and resolves `element="…"` references
...
- Aurora has undocumented app-side behaviour for certain IDs (`ID_INTERNAL_*` are helpers that
  ...
  this. HeroForge treats them as ordinary elements — the importer flags any it cannot resolve.
...
that maps almost directly onto the HeroForge character model — plus inline base64 portraits, a
...
`hf validate` run against a full local checkout of AuroraLegacy/elements, 2026-09-09:
...
- **45 are `ID_INTERNAL_*`** — elements Aurora's app materializes itself, which no XML file
  declares. Exactly the class of thing `core/internal.xml` exists to patch around. HeroForge
